...
- **世界图三态状态填充（2026-08，真实形状版）**：地块状态标记从固定六边形 PNG 图标改为按 tile 真实顶点（`WorldGrid.GetTileVertices`）重建单格 mesh 的半透明填充（橙/蓝/灰，向中心收缩 10% 留边缘给原版描边）。回归点：①填充完全贴合各 tile 实际形状与角度（含五边形）；②原版选中（白）/悬停（黄）描边不被遮挡、与填充无视觉冲突；③三态颜色正确（有人/无人/休眠切换即时）；④点击地块仍可选中出 gizmo（查看地图/休眠/删除）；⑤图删除后无残留填充（mesh 缓存释放）。
