...
2. **但存在一个必须处理的遮挡问题**：宿主地图的 `MapEdgeClipDrawer.DrawClippers` 在 `Map.MapUpdate` 中**最后**绘制黑色裁剪平面，会覆盖宿主边界外的口袋地图区域。处理方案：
   ...
   - **方案 B（推荐）**：Harmony patch `MapEdgeClipDrawer.DrawClippers`，在绘制时跳过/调整与相邻地块接缝重叠的裁剪平面。**注意：跳过范围必须覆盖对端地图的整个可见 footprint，而不仅仅是接缝那一小条**——因为当玩家 Pawn 移动到当前地图边界时，对端地图的大部分区域位于宿主边界之外，若只跳过接缝，对端地图在宿主边界外的部分仍会被宿主裁剪平面涂黑，无法完整显示在玩家视野内。裁剪跳过的判定应以"该裁剪平面是否与任何已加载/可见的对端地图 footprint 相交"为准，而非"是否与接缝线相交"。
...
5. **对最小技术原型（第 2 步）的意义**：原型第 2 条"将相邻地图的大部分区域放置在殖民地地图边界之外"在渲染上可行，但必须同时处理宿主地图的裁剪平面遮挡，否则边界外的部分会被涂黑。原型应验证方案 B（patch `MapEdgeClipDrawer`，跳过范围覆盖对端地图整个 footprint），并验证接缝重叠带内传送点对的重合与无缝转移。
...
- **裁剪覆盖整个 footprint**：当 Pawn 走到殖民地地图边界时，对端地图在宿主边界外的部分必须完整显示在视野内（不被宿主 `MapEdgeClipDrawer.DrawClippers` 涂黑）。验证方案 B 的裁剪跳过范围覆盖对端地图整个可见 footprint，而非仅接缝线。
- **接缝重叠带**：对端地图与本地地图在接缝方向上有物理重叠带，本端传送点与对端传送点在宿主坐标（drawPos）上重合于同一 tile。
- **无缝转移**：Pawn 走到本端传送点后，无缝传送到对端地图相同 drawPos 的 tile 上的对端传送点，视觉上位置不跳变。
...
- VMF 能否显示并操作位于宿主地图边界之外的 Pocket Map？——**能**。VMF 把口袋地图作为宿主地图上的 Thing 绘制（`DrawAt`/`DynamicDrawPhaseAt` + `Graphics.DrawMesh` 带 drawPos 偏移），绕开 `Map.MapUpdate` 的 `Find.CurrentMap == this` 限制和 `MapDrawer` 的 ViewRect 裁剪；选取/移动命令通过 `ToVehicleMapCoord` 反查，与边界无关。**但**宿主地图的 `MapEdgeClipDrawer.DrawClippers` 会在最后绘制黑色裁剪平面遮挡边界外区域，需通过 patch `MapEdgeClipDrawer`（方案 B）或 Thing 绘制 + 自绘裁剪（方案 C）处理（详见 1.10）。
