    ...
    (
        ...
        lambda name, animal, tgt: {
            "殖民者": {
                ...
                "preload": {"spot": "否（已实现：CheckPawnGoto 排除集 = 接缝带 Band 全三圈 ∪ 传送点格（2026-08 扩，与 exit 标记集同口径）——征召 goto 接缝带任意格 = 撤离意图，不触发对端生成）",
                            "band": "是（已生效：playerForced 分支；2026-08 修正：接缝带 Band 三圈非传送点格与 exit 标记集同口径排除——征召 goto 该处 = 撤离意图到站离场，白预加载对侧图的口径缺口已封）"},
