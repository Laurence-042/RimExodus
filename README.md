...
- **直接跨图行走**：接缝处铺传送点，Pawn 走到接缝即无感切换到相邻地图；玩家下令（移动/攻击/开采/搬运等）可直接指向邻图区域。征召的殖民地机械族同样可跨缝下令——机械师指挥半径按无缝世界连续距离评估（overseer 站在缝边即可指挥缝对面，机械族过缝后连续距离内仍可继续指挥；机械师自己过缝也不会解除机械族的征召状态）；Escort 工作模式的机械族会跟随 overseer 跨缝，不会因目标在邻图而卡死报错。
...
| Mod | 状态 |
|-----|------|
| **Geological Landforms**（实测 1.7.13.1） | 完整适配，三轮：①分帧增量生成路径软反射复刻其生成上下文（其 Harmony patch 挂在原生同步管线上，分帧路径天然绕过）；②启动时把我们的地块 parent 注册进其 `IgnoredWorldObjects` 白名单——否则 GL 会把无派系的地块 parent 当作外来 site，把该地块的全部 landform 概率归零（症状：营地图 landform 缺失且被永久写入空数据、地块存在期间预览丢 landform）；③**河流地貌适配（2026-09，不再需要禁用河流地貌）**——GL 的河流在地块接缝处穿越点对齐（进出端点钉到两侧一致的穿越位置，并让河流寻路精确抵达而非"差不多就停"——此前会受寻路的提前接受半径影响在缝旁错开最多约 30 格），河水也不再污染接缝快照（对端地面不会被旧图河水覆盖）。GL 1.7.13 起改用延迟加载，兼容注册已改为在地图生成前自动就绪、不依赖 mod 排序。已知边界：合流/分流类地貌中，分支点贴近目标边的支流可能没有足够空间转向对齐穿越点——该支流保留 GL 原生走向（错位），个别情况下合流图一侧的支流甚至不会延伸到穿越点附近（生成器无法在剩余空间内同时满足多条支流的走向，接缝过渡带只能部分吸收）。适配只对**新生成**的地图生效；旧档里已生成过的河图不变。 |
...
| **Combat Extended**（参考 16.7.3.0） | 适配（特殊弹道已实现，待游戏内回归）：普通枪械、手雷、迫击炮、瞬发射线、制导弹、CIWS、纯格压制与 Ability 发射路径可跨图工作；Pawn/NPC 与 snap/non-snap 炮塔的射程、索敌、CE 命中报告和弹道交接按真实两图数据计算。软检测、无 CE 编译依赖；CE 世界炮击仍交还 CE 原生流程。详见 [Combat Extended 跨图射击适配](doc/CombatExtended跨图射击适配.md)。 |
...
### 跨图战斗
- 爆炸 AoE 不跨缝（含迫击炮/手雷：弹丸可越缝，弹着后只在落点地图按原生范围结算）。
    - 设计如此：我不想patch太多导致兼容性下降
- Combat Extended 的特殊弹道兼容仍待游戏内逐项回归；CE 世界炮击不进入地图接缝弹道层，继续使用 CE 原生世界对象流程。
- **袭击外缘生成的边界**：从邻图行军过来的袭击者不会绑架/偷窃（与跨缝追击的敌人同语义）；袭击超时计时包含行军时间；行军途经图上有你的殖民者/建筑时会先与本地交战（涌现行为）；邪教徒围攻与访客/旅行者暂不参与外缘进场。
    - 设计如此：邪教徒围攻自带"边缘分批布点+吟唱"机制，跨图化难度剧增。近战敌人（发狂动物/食尸鬼/奇美拉/近战袭击者）已支持跨图索敌——锁定邻图目标后桥接行军过缝再打
...
- **武器射程曲线映射**（默认关，见设置项）的边界：
    - CE 武器射程同样映射（CE 最终射程在映射值上再乘其弹药/附件系数），但 CE 命中走自建散布模型、没有原版精度档概念（距离惩罚按新射程自动归一，无需适配）；CE 迫击炮 75 格直瞄/间瞄分界是 CE 硬编码，不随曲线移动。
    - 原版恶劣天气的射程截断仍按原版语义生效；武器最小射程不映射；以 `rangeStat`（按属性定义射程）定义射程的罕见 verb 与挂在小体积 Def（hediff/ability）上的罕见弹丸 verb 不在映射范围。
    - 射程选中圈超过 90 格时原版不再绘制（纯视觉，原版保护）。
...
### 地图生命周期
- **前哨保留（封存/重放）**：有**居住区（居住区）**的地块图到删除距离时不再被删除，而是**封存**——居住区内的建筑、地板、屋顶与地面物资被记录成一份极小的稀疏数据，地图本体从内存卸载（世界图该地块显示青绿色）；你再回来时地图重新生成，你的前哨会在其上重建（居住区内采空的山体也不会复原）。居住区随建造自动扩展，也可以自己用居住区画笔圈定保留范围（"画居住区 = 画保留区"）；只有居住区内的东西会被保留。
...
| Built-in ocean map support | 开（需重启） | 补齐原版 Ocean 的深海地形与基础晴雨雾雷雨，跳过陆地出生点/散布物/野生动物，同时避免海岸补铺在海洋中央生成浅水条带。安装专门海洋地图 mod 时可关闭，重启后 RimExodus 不再应用这些 Ocean 专用行为；无缝边界、跨图旅行与通用接缝揭雾不受影响。只影响之后新生成的海洋地图 |
...
| Walk-in raids arrive from the outer edge | 开 | 步行来袭的敌人（步行袭击者、发狂动物、食尸鬼、蹒跚怪、奇美拉等）从对侧尚未看见的接缝进场，而非从已加载地图之间的边界凭空出现；敌对步行袭击（含工兵/破墙、分阶段进攻、人类炮击围攻）可在一跳邻图外侧进场并跨缝行军攻来，近战敌人也能跨图索敌并跨缝追击；mod 添加的袭击事件同样默认跨图（无法识别的策略降级为普通突击迁移，专属策略行为会丢失——下方"配置袭击事件类型…"可按事件改回本图生成）。空投/钻地等非步行进场不受影响；机械体围攻（集群空降进场，属空投类）、灵能仪式围攻（仪式点锚定被袭击图）与邪教徒围攻始终本图。关 = 完全原版落点 |
| Weapon range curve remap | 关 | 武器射程曲线映射：把全部远程武器（含 CE，敌我同规则）的射程按分段线性曲线整体拉伸，原版精度档位距离（Touch/Short/Medium/Long，原版固定 3/12/25/40 格）随曲线一同移动——武器在新最大射程上的命中表现与原来在旧最大射程上相当，属性提示里的档位距离文案同步更新。默认曲线 3→5、12→22、25→45、40→85、上界 60→140：刻意压制短射程武器的增长、拉大长射程武器之间的差异（原版经典套装映射：霰弹 16→29、左轮 26→48、突击步枪 31→61、栓动 37→77、狙击 45→99）——环境尺度不随无缝世界放大，只有远距离交战包络变大。两点之间线性插值（默认曲线下 32.5→65），超过上界按其比例连续缩放。下方按钮打开曲线编辑器（可改值、曲线预览、试算）。即时生效、关闭即还原；边界细节见"跨图战斗"已知限制 |
...
| Diagnostic logs by module（分模块诊断日志） | 全关 | 按模块开关详细日志（10 个模块 + 一个总控）：生成与地形 / 传送与边界 / **远行队离场诊断（[caravan-exit]）** / **天气域** / 休眠与性能 / 跨图战斗 / **Combat Extended 专项诊断** / 兼容层 / 据点与贸易 / 核心与图管理。CE 专项日志对移动弹丸只记录发射、跨缝交接和最终命中三个离散事件，instant RayCast 每束一条摘要；不记录每 tick/每帧的射线逐格、制导、索敌或 UI 判定，避免日志洪泛触发 RimWorld 抑制。警告/错误与低频生命周期心跳不受开关控制、始终记录。 |
...
| `Generation/` | 地图生成管线：六边形裁切、void、接缝混合、分帧增量生成 | 四个注入 genStep（`GenStep_SeamlessTile` 389 备份+铺 void / `GenStep_SeamOverride` 392 接缝混合 / `GenStep_EnterSpots` 1490 铺传送点 / `GenStep_CoastalEdgeFill` 230）、`SeamlessPolygonGeometry`（接缝带几何唯一实现）、`SeamlessTerrainFill`、`SeamlessSeamOverride`（实时地图/基础快照提供器与卷积混合）、`SeamlessOceanMapSupport`（可关闭并于重启生效的原版海洋地图补全）、`SeamlessBaseSnapshotData`（原生图三层快照的序列化载体）、`SeamlessSnapshotRegenerator`（读档后为所有实际参考邻图按需补快照）、`IncrementalMapGenerator`（分帧增量生成）、`MapGenerationProgressUI`、`SeamlessEnterSpotPlacer` + `CompSeamlessTileEnterSpot`（传送点铺设与标记）、配套 patch：河流/道路对齐（`Patches_TileMutatorRiver`/`Patches_GenStepRoads`）、建筑选址（`Patches_BuildingPlacement`）、揭雾分径（`Patches_GenStepFog`）、海洋陆地步骤过滤（`Patches_OceanGeneration`）、地形守卫（`Patches_TerrainGrid`）、生成计时与互斥（`Patches_MapGenTiming`/`Patches_IncrementalMapGen`）、`Patches_GenConstruct`、远行队进图出生点（`Patches_CaravanEnterMap`） |
...
| `Combat/` | 跨图索敌与射击 | `SeamlessCrossMapSight`（分段 LOS）、`SeamlessCombatCoords`（统一坐标与归属路由）、`SeamlessCrossMapCellTarget`（纯格目标的真实地图上下文）、`SeamlessDirectFireSupport`（原版/第三方 projectile 能力入口）、`Patches_CombatTargetSearch`（索敌两层模型跨图化）、`Patches_CombatTargeter`（武器/Ability 邻图点选）、`Patches_CombatTargeting`（TryCastShot 门/ShotReport）、`Patches_Projectile`（含原版迫击炮的弹丸缝交接与连续更新）、`Patches_CombatVisuals`（朝向/瞄准角/连线等视觉修正）、`SeamlessRaidOuterSpawn` + `Patches_RaidOuterSpawn`（袭击外缘生成：步行袭击从外侧接缝进场，敌对步行袭击可在一跳邻图外侧进场并行军跨缝攻来）、`SeamlessRangeCurve` + `Patches_RangeCurveAccuracy`（武器射程映射曲线：远程 verb def 变异唯一入口 + 原版精度锚点距离逆映射）、`RangeCurveEditorDialog`（曲线编辑器：五行锚点编辑/预览/试算） |
...
- [SteamWorkshop/](doc/SteamWorkshop/) — 创意工坊文案稿：介绍页（[中文](doc/SteamWorkshop/SteamWorkshop介绍-中文.md) / [English](doc/SteamWorkshop/SteamWorkshop介绍-English.md)，中英分离的 BBCode）与 `discussions/` 下的讨论区帖（兼容性详解 / 已知限制 / 性能说明）；`doc/讨论区-关于卸载.txt` 为卸载帖稿。事实以本 README 为源。
