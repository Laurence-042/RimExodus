...
- 世界地图上选中地块图标可手动**休眠**（手动休眠不按距离自动唤醒，只有进图/命令 pawn 走近其接缝才唤醒，且随存档保留；删除距离策略照常）或**删除**（确认后销毁重建）；家园图不受手动休眠/删除影响。地块图标三态：**实心橙 = 活跃有人 / 实心蓝 = 活跃无人 / 灰色空心 = 休眠**（蓝橙为色盲友好配色，休眠另以空心形状冗余区分）。
