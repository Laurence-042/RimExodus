...
| Save base terrain snapshots | 开 | 把每张原生地图接缝裁切前的地形/岩石/屋顶三层快照以紧凑格式存入存档，供"恢复原版兼容模式"按钮在任何会话按真实地形恢复（无快照的图只能就近复制边界地形，观感可能奇怪）。会增大存档体积；长玩在意体积可关——关闭需二次确认，并可顺带清除当前存档已有快照。见下方"卸载须知" |
