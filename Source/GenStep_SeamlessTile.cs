...
{
    /// <summary>
    /// 无缝地块地图的多边形 void 裁切 + snapshot 备份（order=1802）。
    ...
    /// order=1802，在 Fog(1500) 之后执行——此时 Plants/Animals 已 spawn，void 格上的实体由
    /// ApplyPolygonTerrain 的 ClearThingsOnCells/EvacuatePawnsOnCells 事后清理（Patch_GenStepRocksFromGrid
    /// 已在 order=200 提前清岩石，故此处主要清理植物/物品）。
    ///
    /// **snapshot 备份点选在最晚（1802）**：此前锚点家园 A 走 Harmony Postfix（order=220）备份太早，
    /// 之后 Roads/Plants 等仍会改地形导致 snapshot 与最终地形漂移。改为 genStep 后 A/B 统一在 1802 备份，
    /// 漂移消除（snapshot 已是接近最终的地形）。
    ///
