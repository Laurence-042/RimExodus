...
{
    /// <summary>
    /// 无缝地块地图的多边形 void 裁切（阶段4a：真实地形 + void 裁切）。
    ...
    ///
    /// 实际逻辑（备份 snapshot + 铺 void）已抽到 <see cref="SeamlessTerrainFill.BackupSnapshotAndApplyVoid"/>，
    /// 与锚点家园的 <c>Patch_GenStep_MutatorPostTerrain</c> Postfix 共用同一逻辑体（归一）。
    /// </summary>
