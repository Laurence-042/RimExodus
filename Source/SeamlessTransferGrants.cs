...
namespace RimExodus
{
    ...
    internal static class SeamlessTransferGrants
    {
        ...
        internal static void NotifyPawnTransferred(Pawn pawn, Map departureMap, Thing departureSpot, Map arrivalMap, Grant grant)
        {
            ...
            switch (grant.Kind)
            {
                ...
                case GrantKind.Follow:
                    // 落地 NPC 无 lord/duty；驯养动物留在主人身边无需兜底，其余登记游荡宽限。
                    if (!SeamlessBoundaryRules.IsColonyAnimal(pawn))
                    {
        ...
        /// <summary>Bridge 续程：跨图落地后继续走向玩家点击的最终目的地（原 ContinueCrossMapMove 逻辑）。</summary>
        ...
        /// <summary>
        /// 追击者扫描（行为表行 15/23/31，待办④）：目标刚跨图，出发地图上正近战攻击它的 NPC 战斗体
        /// 借同一传送点跨图追击（玩家不能靠跨图甩掉追兵）。走位 Goto 无跨图意图判据不激活；
        /// AttackStatic 恒站立不会踩格——均不扫。
        /// </summary>
        private static void MarkPursuers(Pawn target, Map departureMap, Thing spot)
        {
            foreach (var other in departureMap.mapPawns.AllPawns)
            {
                ...
                if (job == null || job.def != JobDefOf.AttackMelee || job.targetA.Thing != target) continue;
