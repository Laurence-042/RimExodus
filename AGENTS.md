...
- Geological Landforms：分帧路径复刻其 Prepare/Cleanup 上下文；RimExodus parent 必须登记进 GL 的 IgnoredWorldObjects。GL 河流通过 Path tree 钉位，不能回退到离散函数场 warp 或生成后地形修补。GL 河缝对齐 = v9 端点钉位（PathTracer.Trace Prefix，含叶段 AngleTenacity 放宽至地貌自设 AngleLimitAbs 上限内）+ v10 `PathFinder.FindPath` Prefix 接受收紧（钉位目标 TargetAcceptRadius→~1.5 格、PlanarTargetFallback→0——错开主体是提前接受而非转向半径）。**禁止事后位移 A\* 结果**（v10a 节点表扭曲与烘焙碰撞处理器互搏致误差逐轮放大直至 stub 截断、河流整体消失，已撤销）；亦勿用 GridValueSupplier 委托包装（Vector2d 无编译引用需 emit）或 Segment 拆段预弯（无 Target 中段落入梯度跟随模式 + 单节点越界前科）。GL 1.7.13 起主程序集经 LunarLoader 懒加载（Assemblies 只有 LunarLoader.dll，真程序集在 Lunar/Components/）——GL 类型解析必须经 `SeamlessLandformsCompat.EnsureResolved` 可重试，最终注册兜底在生成入口（`EnsureNeighborSnapshots` 漏斗 → `EnsureRegisteredForGeneration`，原生/分帧两路径全覆盖）；勿回退为 RimExodusMod 构造器时点一次性解析（2026-09 教训：ctor 时点解析对 mod 列表顺序敏感，静默失效时河流地貌 tile 两边河 mutator 都不跑、地块图无河无地貌）。
